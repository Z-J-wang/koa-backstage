...
class Service {
	...
	async createOne(newRecord, token) {
		...
		if (!jwt.hasAuth(newRecord.auth, token)) {
			...
			newRecord.password.length < 6 &&
			newRecord.password.length > 18
		) {
			res.code = 3000;
	...
	async getUserinfo(token) {
		let ret = await this._dao.findOne({ token: token });
		if (ret) {
			return { code: 1000, data: ret };
		} else {
			return { code: 5000, msg: '数据获取失败' };
		}
	}
}
